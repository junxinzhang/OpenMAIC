...
export async function register(): Promise<void> {
  ...
  const { isBillingEnabled } = await import('@/lib/billing/config');
  if (isAuthEnabled() && !process.env.DATABASE_URL?.trim()) {
    throw new Error('DATABASE_URL is required when Edu accounts are enabled');
  }
  if (isBillingEnabled() && !isAuthEnabled()) {
    throw new Error('Edu billing requires verified accounts');
  }
  if (!isAuthEnabled()) warnIfAccessCodeIsUnset(process.env.ACCESS_CODE);
  ...
  const { startBillingReconciliationSchedule } = await import('@/lib/maintenance/billing-schedule');
  const billingSchedule = startBillingReconciliationSchedule();
  ...
  const shutdown = (): Promise<void> => {
    shutdownPromise ??= (async () => {
      ...
      try {
        await billingSchedule?.stop();
      } catch (error) {
        console.error('[instrumentation] Billing reconciliation drain failed', error);
      }
      try {
